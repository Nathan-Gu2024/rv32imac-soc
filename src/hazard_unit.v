...
module hazard_unit (
    // For load-use stall
    input wire id_ex_mem_read, // ADD THIS: 1 if instruction is actually a load, 0 for bubbles
    input wire [4:0] id_ex_rd,
    input wire [1:0] id_ex_wb_sel, // (You can keep this if needed elsewhere, but no longer used for stalls)
    input wire [4:0] if_id_rs1, if_id_rs2,
    ...
    always @(*) begin
        // Load-use stall (Now explicitly filtering out bubbles!)
