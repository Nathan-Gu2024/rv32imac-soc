...
module tb_amo_test;
    ...
    initial begin
        $readmemh("test_amo_mem.hex", ddr_mem);
    end
    ...
    initial begin
        ...
        // old values
        check("swap_old", 10, 32'd10);
        check("add_old ", 11, 32'd10);
        check("xor_old ", 12, 32'd10);
        check("and_old ", 13, 32'd12);
        check("or_old  ", 14, 32'd8);
        check("min_old ", 15, -32'd5);
        check("max_old ", 16, -32'd5);
        check("minu_old", 17, 32'd20);
        check("maxu_old", 18, 32'd20);

        // new memory values (read back via lw)
        check("swap_new", 19, 32'd99);
        check("add_new ", 20, 32'd15);
        check("xor_new ", 21, 32'd12);
        check("and_new ", 22, 32'd8);
        check("or_new  ", 23, 32'd11);
        check("min_new ", 24, -32'd5);
        check("max_new ", 25, 32'd3);
        check("minu_new", 26, 32'd7);
        check("maxu_new", 27, 32'd20);
